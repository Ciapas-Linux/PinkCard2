...
History: PJN / 10-04-2000 Now includes full support for palettes.
         ...
         PJN / 01-08-2001 1. Now includes support for saving back to a bmp file
...
BOOL CIJLBitmap::Save(LPCTSTR lpszPathName)
{
  BOOL bSuccess = FALSE;
  if (m_pPicture)
  {
    ...
    {
      long nSize = 0;
      hr = m_pPicture->SaveAsFile(pStream, TRUE, &nSize);
      if (SUCCEEDED(hr))
      {
        HGLOBAL hGlobal;
        ...
        if (SUCCEEDED(hr))
        {
          ...
          {
            f.Write(pvData, nSize);
            bSuccess = TRUE;
          }
          else
            TRACE(_T("Failed to open file %s, Error:%x\n"), lpszPathName, ::GetLastError());

          //Decrement the reference count on the BLOB
          GlobalUnlock(hGlobal);
        }
      }
      else
      {
        TRACE(_T("Failed in call to IPicture::SaveAsFile, Error:%x\n"), hr);
      }

      //Release the stream interface not that we are finished with it
      pStream->Release();
    }
    else
      TRACE(_T("Failed in call to CreateStreamOnHGlobalfile, Error:%x\n"), hr);
  }
